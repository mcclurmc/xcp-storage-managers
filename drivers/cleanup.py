...
class XAPI:
    ...
    CONFIG_SM = 0
    CONFIG_OTHER = 1
    CONFIG_NAME = { CONFIG_SM: "sm-config", CONFIG_OTHER: "other-config" }
...
class FileSR(SR):
    ...
    def cleanupCache(self):
        ...
        for cacheFile in cacheFiles:
            ...
            if valid and rec["managed"] and not rec["allow_caching"]:
                Util.log("Cache %s: caching disabled" % uuid)
                valid = False

            if valid:
                Util.log("Cache %s: valid" % uuid)
                continue
            ...
            cacheLock.acquire()
            try:
                self._cleanupCache(uuid)
            finally:
                cacheLock.release()

    def _cleanupCache(self, uuid):
        rec = self.xapi.getRecordVDI(uuid)
        if rec and (rec["allow_caching"] or not rec["managed"]):
