...
class TapCtl(object):
    ...
    @classmethod
    def unpause(cls, pid, minor, _type = None, _file = None):
...
def mkdirs(path, mode=0777):
    if not os.path.exists(path):
        ...
        assert parent != path
        if parent: mkdirs(parent, mode)
        if subdir: os.mkdir(path, mode)
...
class Tapdisk(object):
    ...
    def unpause(self, _type=None, path=None):
        ...
        TapCtl.unpause(self.pid, self.minor, _type, path)
...
class VDI(object):
    ...
    def _tap_activate(phy_path, vdi_type, sr_uuid, writable):
        ...
        if not tapdisk:
            ...
            # NB. disabled until blkback got frame pools as well.
    ...
    def reset_leaf(self):
        ...
        parent_uuid = vhdutil.getParent(self.target.vdi.path,
                FileVDI.extractUuid)
        if not parent_uuid:
            util.SMlog("ERROR: VDI %s has no parent, not resetting" % \
    ...
    def _setup_cache(self, session, sr_uuid, vdi_uuid, local_sr_uuid,
        ...
        if not prt_tapdisk:
            blktap = Blktap.allocate()
            try:
                blktap.set_pool_name("lcache-parent-pool")
                prt_tapdisk = \
                    Tapdisk.launch_on_tap(blktap,
                                          read_cache_path, 'vhd',
                                          rdonly=False,
