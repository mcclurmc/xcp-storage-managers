...
# Now for the main functions:    
    ...
    def attach(self, sr_uuid):
        ...
        if self.dconf.has_key('type'):
            if self.dconf['type']=='cifs':
                ...
                mountcmd=["mount.nfs", location, self.mountpoint]
        else:
            # Not-so-legacy mode:
    ...
    def scan(self, sr_uuid):
        ...
        if self.dconf.has_key("legacy_mode"):
	    # Out of all the xs-tools ISOs which exist in this dom0, we mark only one
	    # as the official one. 
	    ...
	    # Everything that looks like an xs-tools ISO but which isn't the primary one will
	    # also be renamed "Old version of ..."
	    for location in scanrecord.all_xenapi_locations():
		vdi_ref = self.session.xenapi.VDI.get_by_uuid(scanrecord.get_xenapi_vdi(location)['uuid'])
